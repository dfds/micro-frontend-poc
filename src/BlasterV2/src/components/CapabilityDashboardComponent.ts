...
export default class CapabilityDashboardComponent extends WebComponent {
    ...
    @property({type: TemplateResult}) errorMessage = html``;
    ...
    hasError() : Boolean {
        return this.errorMessage.getHTML.valueOf() !== "".valueOf();
    }
    ...
    render(): TemplateResult {
        ...
        <div class="capabilityList">
            <h1 @click=${this.interactButton}>${this.enabled}</h1>
            <h1>${this.currentUser}</h1>


            ${this.hasError ? html`<span>${this.errorMessage}</span>` : ``}
            ${this.enabled ? html`
    ...
    firstUpdated(_propertyValues : any): void {
        ...
        .then((resp : any) => {
            ...
            if (resp.status === 200) {
                ...
                this.errorMessage = html`<div style="background-color: #dbb9a1; display: flex; justify-content: center;"><p style="text-align: center;">Something went wrong:<br>${resp.status}<br>${resp.data}</p></div>`;
            }
        })
        .catch((err : any) => {
            console.log(err);
            console.log("ERR, DIDN'T GET SUCCESS RESPONSE FOR CAPABILITIES");
            this.errorMessage = html`<div style="background-color: #dbb9a1; display: flex; justify-content: center;"><p style="text-align: center;">Something went wrong:<br>${err}</p></div>`;
        });
    }
